...
export function TwoColumnPage({
  ...
  return (
    ...
    >
      {/* Sidebar */}
      <div style={{ background: theme.sidebar || theme.primary, padding: '28px 18px', color: textColor.body }}>
        {isFirst && (
          <>
      ...
      <div style={{ padding: '28px 26px' }}>
        {isFirst && (
          <div style={{ marginBottom: 22 }}>
            <div style={{ fontSize: fs * 2, fontWeight: 700, lineHeight: 1.3, marginBottom: 3, textAlign: 'center' as React.CSSProperties['textAlign'] }}>
...
export function TwoColumnLayout({
  ...
  const sideSectionNodes: SidebarSection[] = sideKeys.flatMap((key) => {
    ...
    if (key === 'languages') {
      ...
      return [{
        key, content: (
          <SideSection key={key} title="Ngoại ngữ" fontSize={fs * 0.75} titleColor={textColor.heading} borderColor={`${accentColor}30`} addButton={addBtn}>
    ...
    if (key === 'education') {
      ...
      return [{
        key, content: (
          <SideSection key={key} title="Học vấn" fontSize={fs * 0.75} titleColor={textColor.heading} borderColor={`${accentColor}30`} addButton={addBtn}>
