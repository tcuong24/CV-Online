...
export function StylePanel({
  ...
  const font =
    FONT_OPTIONS.find((f) => f.id === style.fontId) ?? FONT_OPTIONS[0];
  const { undo, redo, pastStates, futureStates } = useStore(
    useCvEditorStore.temporal,
    (state) => state,
  );
  ...
  const handleFileImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const errMsg =
        err?.response?.data?.message || "Có lỗi xảy ra khi nhập file!";
    ...
    >
      ...
      <ToolGroup>
        ...
        >
          <Sparkles
            size={18}
            className={!aiPanelOpen ? "animate-pulse text-violet-500" : ""}
          />
        </Button>
      </ToolGroup>
      ...
      <ToolGroup>
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="start"
            className="p-3 border shadow-xl rounded-lg bg-white w-56"
          >
              ...
              {COLOR_THEMES.map((t) => (
                <DropdownMenuItem
                  ...
                  style={{
                    ...
                    border: style.themeId === t.id ? "2px solid #000" : "none",
                  }}
                />
              ))}
            </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 px-3 h-9 hover:bg-slate-50 rounded-md transition-colors group min-w-[120px] ml-1">
              ...
              <ChevronDown
                size={12}
                className="text-slate-400 group-hover:text-slate-600 ml-auto"
              />
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent
            align="start"
            className="p-1 border shadow-xl rounded-lg bg-white w-64 max-h-[50vh] overflow-y-auto"
          >
            {FONT_OPTIONS.map((f) => (
              <DropdownMenuItem
                key={f.id}
      ...
      <ToolGroup>
        ...
        <div className="flex items-center gap-2 px-2">
          <Button
            ...
            onClick={() => set("fontSize", Math.max(10, style.fontSize - 1))}
          >
            <Minus size={14} />
          </Button>
          <span className="text-[13px] font-bold text-slate-700 min-w-[20px] text-center">
            {style.fontSize}
          </span>
          <Button
            variant="ghost"
            size="icon"
            className="w-6 h-6 text-slate-400"
            onClick={() => set("fontSize", Math.min(20, style.fontSize + 1))}
          >
            <Plus size={14} />
          </Button>
        </div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="start"
            className="p-1 border shadow-xl rounded-lg bg-white w-32"
          >
      ...
      <style jsx>{`
        .style-toolbar::-webkit-scrollbar {
          display: none;
        }
      `}</style>
    </div>
